...
class hsenews:
	...
	url = 'https://www.m24.ru/tag/%D0%BE%D0%B1%D1%80%D0%B0%D0%B7%D0%BE%D0%B2%D0%B0%D0%BD%D0%B8%D0%B5'
	lastkey = ""
	lastkey_file = ""
	...
	def news_info(self, uri):
		link = self.host + uri
		r = requests.get(link)
		html = BS(r.content, 'html.parser')

		# parse poster image url
		poster = re.match(r'background-image:\s*url\((.+?)\)', html.select('.image-news-logo > .image')[0]['style'])
